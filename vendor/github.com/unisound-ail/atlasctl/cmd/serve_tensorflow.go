...
func NewServingTensorFlowCommand() *cobra.Command {
	...
	var command = &cobra.Command{
		Use:     "tensorflow",
		Short:   "Create tensorflow serving job to deploy and serve machine learning models.",
		Aliases: []string{"tf"},
...
func serveTensorFlow(args []string, serveTensorFlowArgs *ServeTensorFlowArgs, client *kubernetes.Clientset) (err error) {
	...
	if exist {
		return fmt.Errorf("the job %s is already exist, please delete it firstly. use 'atlasctl delete %s'", name, name)
	}
	...
	return util.InstallRelease(name, namespace, serveTensorFlowArgs, tfservingChart)
}
