...
func NewTrafficRouterSplitCommand() *cobra.Command {
	var (
		createArgs runTrafficRouterSplitArgs
	)
	...
	var command = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			err = runTrafficRouterSplit(client, istioClient, &createArgs)
			if err != nil {
				fmt.Println(err)
				os.Exit(1)
			}
	...
	command.Flags().StringVar(&createArgs.ServingName, "servingName", "", "the serving name")
	command.Flags().StringVar(&createArgs.Namespace, "namespace", "", "namespace (default \"default\")")
	command.Flags().StringVar(&createArgs.Versions, "versions", "[]", "Model versions which the traffic will be routed to, e.g. [1,2,3]")
	command.Flags().StringVar(&createArgs.Weights, "weights", "[]", "Weight percentage values for each model version which the traffic will be routed to,e.g. [70,20,10]")
...
func runTrafficRouterSplit(client *kubernetes.Clientset, istioClient *rest.RESTClient, createArgs *runTrafficRouterSplitArgs) (err error) {
	preprocesObject, err := createArgs.preprocess(client)
	if err != nil {
		return err
	}
